...
def REQ_Data(host, sitekey):
    global proxies, proxy, proxylist
    while True:
        ...
        with httpx.Client(proxies=proxies) as client:
            try:            
                ...
                r = client.get(f"https://hcaptcha.com/checksiteconfig?host={host}&sitekey={sitekey}&sc=1&swa=1", headers=headers, timeout=4)
                if r.json()["pass"]:
                    return r.json()["c"]
                else:
                    return False
...
def Get_Captcha(host, sitekey, n, req):
    with httpx.Client(proxies=proxies) as client:
        try:
            ...
            r = client.post(f"https://hcaptcha.com/getcaptcha?s={sitekey}",cookies=cookies, data=data, headers=headers, timeout=4)
            return r.json()
        except Exception as e:
            print(e)
            return False

def bypass(sitekey, host, s):
    try :
